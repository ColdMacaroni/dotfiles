...
M.watch_widget, M.timer = awful.widget.watch("wpctl get-volume @DEFAULT_AUDIO_SINK@", 5, function(_, stdout)
...
M.watch_widget:add_button(awful.button({}, awful.button.names.LEFT, function() awful.spawn("pavucontrol") end))
...
M.update = function(volume, muted)
    ...
    M.watch_widget.markup = ("<span foreground='%s'>%s%% %s</span>"):format(col, volume, icon)
end
